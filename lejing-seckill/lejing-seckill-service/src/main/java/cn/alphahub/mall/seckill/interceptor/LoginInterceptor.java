...
public class LoginInterceptor implements HandlerInterceptor {
    ...
    static {
        URI_WHITELIST_ANT_MATCH_PATTERN.add("/seckill/current/**");
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        if (Objects.nonNull(attribute) && (attribute instanceof Member)) {
            ...
            response.getWriter().println("<script>alert('请先进行登录，再进行后续操作！');location.href='" + AuthConstant.LOGIN_PAGE_URL + "'</script>");
            return false;
        }
    }
